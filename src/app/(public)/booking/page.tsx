...
export default function BookPage() {
  ...
  const dynamicFields = adminDefinedFields.filter((field) => {
    ...
    // Prevent duplicate rendering of built-in fields that are always present.
  ...
  const handleSubmit = async () => {
    ...
    if (requiredOverrides.name && !form.name.trim()) {
      setError('Full Name is required')
      setSubmitting(false)
      return
    }
    if (requiredOverrides.email && !form.email.trim()) {
      setError('Email Address is required')
      setSubmitting(false)
      return
    }
    if (requiredOverrides.phone && !form.phone.trim()) {
      setError('Phone Number is required')
      setSubmitting(false)
      return
    }
    if (requiredOverrides.notes && !form.notes.trim()) {
  ...
  // Build step list based on whether services exist
  ...
  return (
    ...
    >
      {/* ── Hero + Step Indicator as one unified block ── */}
      <div className="bg-[#0040A8] relative overflow-hidden">
        ...
        {/* Step indicator — sits at the bottom of the blue section, half-overlapping into the content below */}
        {dataLoaded && step < 4 && (
        ...
        {/* Bottom curve / spacer — gives room for the step indicator to overlap */}
        <div className={dataLoaded && step < 4 ? 'h-10' : 'h-6'} />
      </div>

      {/* Main content — top padding accounts for the overlapping step indicator */}
        ...
        <AnimatePresence>
          ...
          {dataLoaded && step === 1 && services.length > 0 && (
            <motion.div key="step1" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
              <h2 className="text-[22px] font-bold text-[#1a1a2e] mb-2 font-app-sans">
                Select a Service
              </h2>
                ...
                <div className="bg-white rounded-2xl border border-[#e8e8e0] p-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <div className="flex gap-1">
                      <button
                        onClick={prevMonth}
                        className="w-8 h-8 rounded-lg border border-[#e8e8e0] bg-white cursor-pointer flex items-center justify-center text-[#6b7280] hover:bg-gray-50 transition-colors"
                      >
                        <ChevronLeft size={16} />
                      </button>
                      <button
          ...
          {dataLoaded && step === 3 && (
            ...
            >
              ...
              <div className="bg-white rounded-2xl border border-[#e8e8e0] p-7">
                <h2 className="text-xl font-bold text-[#1a1a2e] mb-6 font-app-sans">
                  Your Information
                </h2>
