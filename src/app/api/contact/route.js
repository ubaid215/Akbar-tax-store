...
export async function POST(request) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL ?? 'Akbar Tax Store <noreply@akbartaxstore.com>',
...
<body style="margin:0;padding:0;background-color:#D9E8FF;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,Cantarell,sans-serif;">
  <div class="container" style="max-width:600px;margin:20px auto;background-color:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 10px 40px rgba(7,41,113,0.15);">
    ...
    <!-- Header -->
    <div class="header" style="background:linear-gradient(135deg,#072971 0%,#0040A8 100%);color:#FFFFFF;padding:40px 30px;text-align:center;">
    ...
    <div class="content" style="padding:40px 30px;background-color:#FFFFFF;">
      <div class="card" style="background:linear-gradient(135deg,#D9E8FF 0%,rgba(217,232,255,0.5) 100%);padding:30px;border-radius:12px;border:1px solid rgba(0,64,168,0.1);position:relative;">
        ...
        <h2 style="color:#072971;margin:0 0 25px 0;font-size:22px;font-weight:600;display:flex;align-items:center;">
          <span style="background:#0040A8;width:4px;height:20px;border-radius:2px;margin-right:12px;display:inline-block;"></span>
          Contact Details
        </h2>
        ...
        ${[
          ...
          { label: 'Email', value: `<a href="mailto:${email}" style="color:#0040A8;text-decoration:none;font-weight:600;">${email}</a>` },
          { label: 'Phone', value: phone ? `<a href="tel:${phone.replace(/\s/g, '')}" style="color:#0040A8;text-decoration:none;font-weight:600;">${phone}</a>` : '<span style="color:#999;font-style:italic;">Not provided</span>' },
          { label: 'Subject', value: `<span style="background:rgba(0,64,168,0.1);color:#0040A8;padding:4px 12px;border-radius:20px;font-size:14px;font-weight:600;">${subject}</span>` },
        ].map(r => `
        <div class="detail-row" style="display:flex;padding:12px 0;border-bottom:1px solid rgba(0,64,168,0.1);">
          <div class="detail-label" style="min-width:100px;font-weight:600;color:#072971;display:flex;align-items:center;">
            <span style="background:#0040A8;width:8px;height:8px;border-radius:50%;margin-right:8px;opacity:0.7;display:inline-block;"></span>
            ${r.label}
          </div>
          <div class="detail-value" style="flex:1;color:#050505;font-weight:500;padding-left:20px;">${r.value}</div>
        </div>`).join('')}

        <!-- Message -->
        <div style="margin-top:30px;">
          <h3 style="color:#072971;margin:0 0 15px 0;font-size:18px;font-weight:600;display:flex;align-items:center;">
            <span style="background:#0040A8;width:4px;height:16px;border-radius:2px;margin-right:12px;display:inline-block;"></span>
            Message
          </h3>
    ...
    <div class="footer" style="background:linear-gradient(135deg,#050505 0%,#072971 100%);color:#FFFFFF;padding:25px 30px;text-align:center;">
      <div style="margin-bottom:15px;">
        <div style="background:rgba(255,255,255,0.1);width:50px;height:50px;border-radius:50%;margin:0 auto;display:flex;align-items:center;justify-content:center;">
      ...
      <div style="margin-top:20px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.1);">
        ...
        <div style="display:inline-flex;gap:10px;">
          <a href="https://wa.me/message/QJQEJZWC36JKN1" style="background:rgba(255,255,255,0.1);width:35px;height:35px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;text-decoration:none;">
            <span style="color:#FFFFFF;font-size:14px;">📱</span>
          </a>
          <a href="https://www.instagram.com/_akbar_tax_store" style="background:rgba(255,255,255,0.1);width:35px;height:35px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;text-decoration:none;">
            <span style="color:#FFFFFF;font-size:14px;">📸</span>
          </a>
        </div>
      </div>
  ...
  <div style="height:20px;background-color:#D9E8FF;"></div>
    ...
    return NextResponse.json({ message: 'Email sent successfully', success: true, id: data?.id }, { status: 200 });
